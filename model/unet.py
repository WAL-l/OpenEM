...
class HeightstepBlock(nn.Module):
    @abstractmethod
...
class TimestepEmbedSequential(nn.Sequential, HeightstepBlock):
    """
    ...
    def forward(self, x, emb):
        for layer in self:
            if isinstance(layer, HeightstepBlock):
                x = layer(x, emb)
            else:
                x = layer(x)
        return x
...
class ResBlock(HeightstepBlock):
    """
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        hight_embed_dim = model_channels * 4
        self.time_embed = nn.Sequential(
            linear(model_channels, hight_embed_dim),
            nn.SiLU(),
            linear(hight_embed_dim, hight_embed_dim),
        )

        ch = input_ch = int(channel_mult[0] * model_channels)
        self.input_blocks = nn.ModuleList(
            [TimestepEmbedSequential(nn.Conv3d(in_channels, ch, 3, stride=(2, 2, 1),padding=1),
    ...
    def forward(self, x, height):
        ...
        """
        hs = []
        emb = self.time_embed(height_embedding(height, self.model_channels))
        h = x.type(self.dtype)
...
if __name__ == "__main__":
    ...
    #data = np.load("../dataset/train/curve/model/0.npy")
    unet = UNet(
        in_channels=1,
        model_channels=128,
        out_channels=1,
    ...
    # 输入数据
    x = th.randn(1, 1, 64, 64, 32).cuda()
    height = th.randn(1).cuda()

    output = unet(x, height)
    print(output.shape)
